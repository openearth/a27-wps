...
SELECT json_build_object(
    'locationproperties',json_build_object(
						'locatie_id', pf.locatie_id,
	   					'locatie_naam',l.locatienaam_master,
	   					'maaiveld_hoogte',l.maaiveldhoogte_cm_nap,
       					'peilfilter_naam', pf.peilfilternaam,
	   					'peilfilter_id', pfd.peilfilter_id, 
	   					'bovenkantpeil_buis',pfd.bovenkant_peilbuis_meetpunt_cm_nap, 
	   					'bovenkant_filter',pfd.bovenkant_filter_cm_nap, 
	   					'onderkant_filter',pfd.onderkant_filter_cm_nap,
	   					'dlabel_filter',  dl."Deltares_peilfilter_interval_label"::text,
						'pompid_filter', pp.nitg_nummer::text
	                                      ),
...
LEFT JOIN public."Dlabels_draft" dl ON dl.peilfilter_id = pfd.peilfilter_id
LEFT JOIN public.pompproef_draft pp ON pp.peilfilter_id = pfd.peilfilter_id
...
group by pf.locatie_id,
		 ...
		 dl."Deltares_peilfilter_interval_label"::text,
		 pp.nitg_nummer::text
$BODY$;
